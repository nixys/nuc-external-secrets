...
This file is the reference baseline for the `nuc-vault-secret-operator` Helm chart repository.
...
- Do not assume the latest Kubernetes patch has a matching `kindest/node` image for local e2e.
...
## Vault Secret Operator Rules

For this chart repository, keep these assumptions explicit:

- chart rendering support and cluster installability are different concerns
- resource support depends on Vault Secrets Operator CRDs being installed in the cluster
- the chart renders only `VaultAuth` and `VaultStaticSecret` resources
- `vaultAuths` and `vaultStaticSecrets` are name-keyed resource maps
- legacy `vaultAuth` remains supported only for backward compatibility
- shorthand fields and raw `spec` pass-through are both part of the public contract
