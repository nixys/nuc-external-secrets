...
.PHONY: test
test: test-unit test-smoke ## Run local checks
...
.PHONY: test-smoke
test-smoke: ## Run all smoke scenarios, including kubeconform; append SMOKE_ARGS='--scenario example-kubeconform'
	python3 tests/smokes/run/smoke.py $(SMOKE_ARGS)

.PHONY: test-smoke-fast
test-smoke-fast: ## Run smoke scenarios that do not require kubeconform
