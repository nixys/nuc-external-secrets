...
show_help() {
  ...
  echo ""
  echo "Create a kind cluster, install External Secrets Operator via the official Helm chart,"
  echo "and run Helm install/upgrade against the root chart."
...
dump_cluster_state() {
  log_warn "Dumping External Secrets Operator resources from ${CLUSTER_NAME}"
...
verify_release_resources() {
  log_info "Verifying installed External Secrets Operator resources"
  kubectl -n "${E2E_NAMESPACE}" get secretstore e2e-fake-store
  kubectl -n "${E2E_NAMESPACE}" get externalsecret e2e-app
  kubectl -n "${E2E_NAMESPACE}" wait --for=condition=Ready --timeout=120s externalsecret/e2e-app
  kubectl -n "${E2E_NAMESPACE}" get secret e2e-app -o yaml
  echo
}
